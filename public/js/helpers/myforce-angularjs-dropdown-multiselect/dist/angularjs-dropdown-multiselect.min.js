...
directiveModule.directive("mfDropdownStaticInclude", ["$compile", function ($compile) {
    return function (scope, element, attrs) {
        var template = attrs.mfDropdownStaticInclude, contents = element.html(template).contents();
    ...
    return {
        ...
        link: function ($scope, $element, $attrs) {
            ...
            $scope.toggleDropdown = function () {
                $scope.open = !$scope.open, $scope.settings.keyboardControls && $scope.open && (1 === $scope.settings.selectionLimit && $scope.settings.enableSearch ? setTimeout(function () {
                ...
                if ($scope.settings.dynamicTitle && ($scope.selectedModel.length > 0 || angular.isObject($scope.selectedModel) && Object.keys($scope.selectedModel).length > 0)) {
                    ...
                    var totalSelected;
                    return totalSelected = $scope.singleSelection ? null !== $scope.selectedModel && angular.isDefined($scope.selectedModel[$scope.settings.idProp]) ? 1 : 0 : angular.isDefined($scope.selectedModel) ? $scope.selectedModel.length : 0, 0 === totalSelected ? $scope.texts.buttonDefaultText : $scope.texts.dynamicButtonTextSuffix + " (" +totalSelected + ")"
                }
                ...
                if (finalObj = "" === $scope.settings.externalIdProp ? find($scope.options, findObj) : findObj, $scope.singleSelection)clearObject($scope.selectedModel), angular.extend($scope.selectedModel, finalObj), $scope.externalEvents.onItemSelect(finalObj), ($scope.settings.closeOnSelect || $scope.settings.closeOnDeselect) && ($scope.open = !1); else {
                ...
                var nextOption, sourceScope = angular.element(event.target).scope(), parent = event.srcElement.parentNode;
                if ($scope.settings.keyboardControls)if (13 === event.keyCode || 32 === event.keyCode)event.preventDefault(), sourceScope.option ? $scope.setSelectedItem($scope.getPropertyForObject(sourceScope.option, $scope.settings.idProp), !1, !0) : "deselectAll" === event.srcElement.id ? $scope.deselectAll() : "selectAll" === event.srcElement.id && $scope.selectAll(); else if (38 === event.keyCode) {
                ...
                var nextOption, parent = event.srcElement.parentNode.parentNode;
                if ($scope.settings.keyboardControls)if (9 === event.keyCode || 40 === event.keyCode)event.preventDefault(), setTimeout(function () {
                    angular.element($element)[0].querySelector(".option").focus()
                }, 0); else if (38 === event.keyCode) {
                    for (event.preventDefault(), parent.previousElementSibling && (nextOption = parent.previousElementSibling.querySelector("a") || parent.previousElementSibling.querySelector("input")); !nextOption && parent;)parent = parent.previousElementSibling, parent && (nextOption = parent.querySelector("a") || parent.querySelector("input"));
                    nextOption && nextOption.focus()
                } else 27 === event.keyCode && (event.preventDefault(), $scope.toggleDropdown())
